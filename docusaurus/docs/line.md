...
| `hideTooltipAfter`   | `number` | No | Defines a period in ms after which the tooltip should hide automatically.  |
| `hideTooltipOnDragEnd`   | `boolean` | No | Set to true if the tooltip should be hidden immediately when the user stops dragging the chart. (Only dragging, on tap the tooltip remains!)  |
